      ...
      <div class="header-client-info">
        <div class="header-title-container">
          ...
          <h2 class="header-title">
            Edit {{ client.category === "contact" ? "Contact" : "Client" }}
          </h2>
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const fetchClientData = async () => {
      ...
      try {
        const response = await axios.get(
          `https://appv1.fieldseasy.com/directus/items/organization/${clientId}`,
          {
            headers: {
    ...
    const updateMap = () => {
      if (!client.address || !geocoder) return;
    ...
    const searchLocation = () => {
      ...
      geocoder.geocode({ address: client.googleSearch }, (results, status) => {
        if (status === "OK" && results[0]) {
          const location = results[0].geometry.location;
          client.address = results[0].formatted_address;
          client.latitude = location.lat().toFixed(8);
          client.longitude = location.lng().toFixed(8);
          map.setCenter(location);
          map.setZoom(15);
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        console.log(
          `${client.category === "contact" ? "Contact" : "Client"} updated:`,
          client,
        );
        alert(
          `${client.category === "contact" ? "Contact" : "Client"} updated successfully!`,
        );
        router.push("/organization/orgmainui");
      } catch (error) {
        console.error(
          `Error updating ${client.category === "contact" ? "contact" : "client"}:`,
          error,
        );
        alert(
          `Failed to update ${client.category === "contact" ? "contact" : "client"}. Please try again.`,
        );
      } finally {

...
export default {
  ...
  methods: {
    ...
    initializeCurrentTab() {
      ...
      if (tabExists && this.hasAccess(this.tabs.find(tab => tab.name === currentRouteName).meta.roles)) {
        ...
        if (kpiTab && this.hasAccess(kpiTab.meta.roles)) {
          ...
          const accessibleTab = this.tabs.find(tab => this.hasAccess(tab.meta.roles))
          if (accessibleTab) {

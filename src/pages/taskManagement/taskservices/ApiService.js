...
const API_TOKEN = authService.getToken();
    ...
    const API_BASE_URL = 'https://access.sensenservice.com';
...
class ApiService {
  ...
  static async uploadImage(file) {
    try {
      ...
      try {
        ...
        throw new Error(`File upload failed with status ${response.status}. Response is not valid JSON.`);
      }
  ...
  static async fetchTaskById(taskId) {
    const response = await this.request(`/items/tasks/${taskId}?filter[_and][0][tenant][_eq]=${TENANT_ID}`);
    return response.data;
  }
  ...
  static async fetchCustomersBySiteType(siteType) {
    try {
      ...
      console.log(`✅ Found ${response.data.length} customers for site type: ${siteTypeFilter}`);
      return response.data;
    } catch (error) {
  ...
  static async fetchCustomerById(customerId) {
    try {
      const response = await this.request(`/items/customers/${customerId}?filter[_and][0][tenant][_eq]=${TENANT_ID}`, {
        headers: {
          Authorization: `Bearer ${API_TOKEN}`,
        },
      });
  ...
  static async fetchTaskCounts() {
    try {
      ...
      // Calculate counts
      return {
        total: pendingTasks.length + inProgressTasks.length + completedTasks.length,
  ...
  static async fetchEmployeeById(employeeId) {
    ...
    try {
      ...
      // Debug Log
      console.log(`✅ [fetchEmployeeById] Response for ID ${employeeId}:`, response);
  
      return response.data;
    } catch (error) {
      console.error(`❌ [fetchEmployeeById] Error fetching employee ${employeeId}:`, error);
      throw error;
    }
  }
  ...
  static async checkEmployeeAttendance({ employeeId, startDate, endDate }) {
    try {
      ...
      // Check for leave
      const leaveUrl = `/items/attendance?filter[_and][0][date][_between][0]=${start}&` +
          `filter[_and][0][date][_between][1]=${end}&` +
          `filter[_and][1][employeeId][id][_eq]=${employeeId}&` +
          `filter[_and][0][attendance][_iends_with]=leave`;
      
      // Check for absent
      const absentUrl = `/items/attendance?filter[_and][0][date][_between][0]=${start}&` +
          `filter[_and][0][date][_between][1]=${end}&` +
          `filter[_and][1][employeeId][id][_eq]=${employeeId}&` +
      ...
      console.log('✅ [checkEmployeeAttendance] Leave records:', leaveRecords.length);
      console.log('✅ [checkEmployeeAttendance] Absent records:', absentRecords.length);
